const { deployments, getNamedAccounts, ethers, network } = require("hardhat")
const { assert, expect } = require("chai")
const fs = require("fs")
const { developmentChains } = require("../../helper-hardhat-config")

const lowSVGImageuri =
  "data:image/svg+xml;base64,"
const highSVGimageUri =
  "data:image/svg+xml;base64,"

const highTokenUri =
  "data:application/json;base64,"
const lowTokenUri =
  "data:application/json;base64,"

!developmentChains.includes(network.name)
  ? describe.skip
  : describe("DynamicSvgNft", function () {
      let dynamicSvgNft, name, symbol, tokenCounter
      let lowSvg, highSvg, highValue
      beforeEach(async function () {
        deployer = (await getNamedAccounts()).deployer
        await deployments.fixture(["all"])
        dynamicSvgNft = await ethers.getContract("DynamicSvgNft", deployer)
        lowSvg = await fs.readFileSync("./images/dynamicNft/frown.svg", {
          encoding: "utf-8",
        })
        highSvg = await fs.readFileSync("./images/dynamicNft/happy.svg", {
          encoding: "utf-8",
        })
        highValue = await ethers.utils.parseEther("1")
      })

      describe("constructor", function () {
        it("initializes the nft correctly...", async function () {
          tokenCounter = await dynamicSvgNft.getTokenCounter()
          name = await dynamicSvgNft.name()
          symbol = await dynamicSvgNft.symbol()
          assert.equal(tokenCounter.toString(), "0")
          assert.equal(name.toString(), "Dynamic SVG NFT")
          assert.equal(symbol.toString(), "DSN")
        })
      })

      describe("svgToImageURI", function () {
        it("returns a encoded image uri", async function () {
          const imageuri = await dynamicSvgNft.svgToImageURI(lowSvg)
          expect(imageuri.toString(), lowSVGImageuri)
        })
      })

      describe("mintNft", function () {
        it("emits an event and creates a NFT", async function () {
          let highValue = await ethers.utils.parseEther("1")
          await expect(dynamicSvgNft.mintNft(highValue)).to.emit(
            dynamicSvgNft,
            "CreatedNFT"
          )
          const tokenUri = await dynamicSvgNft.tokenURI(0)
          assert.equal((await dynamicSvgNft.getTokenCounter()).toString(), "1")
          assert.equal(tokenUri, highTokenUri)
        })
      })

      describe("tokenUri", function () {
        this.beforeEach(async function () {
          await dynamicSvgNft.mintNft(highValue)
        })
        it("reverts for non-existant token URI", async function () {
          await expect(
            dynamicSvgNft.tokenURI(100)
          ).to.be.revertedWithCustomError(
            dynamicSvgNft,
            "ERC721Metadata__URI_QueryFor_NonExistentToken"
          )
        })

        it("returns a token for existing tokenId", async function () {
          const tokenUri = await dynamicSvgNft.tokenURI(0)
          assert.equal(tokenUri, highTokenUri)
        })
      })
    })
